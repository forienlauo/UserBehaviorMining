...
class CNNTrainer(object):
    ...
    DEFAULT_CONV_DEPTH, DEFAULT_CONV_HEIGHT, DEFAULT_CONV_WIDTH = 2, 5, 5
...
need args:
    ...
    <initial_depth> <initial_height> <initial_width> <initial_channels> <target_class_cnt>
...
""")
    ...
    def fit(argv):
        """训练并评估 cnn 模型
        样本格式(x): 每行样本是一张拉成1维的图片(depth*height*weight*in_channels)
        ...
        _offset, _length = _offset + _length, 5
        initial_depth, initial_height, initial_width, initial_channels, target_class_cnt = \
            map(int, argv[_offset:_offset + _length])
        ...
        _offset, _length = _offset + _length, 3
        conv_depth, conv_height, conv_width = map(int, argv[_offset:_offset + _length])
        _offset, _length = _offset + _length, 1
        ...
        with tf.name_scope('input') as _:
            x = tf.placeholder(tf.float32, shape=[None, initial_depth * initial_height * initial_width], name='x', )
    ...
    def construct(
        ...
        def conv3d(
                x, W,
                strides=(1, CNNTrainer.CONV_STRIDES_D, CNNTrainer.CONV_STRIDES_H, CNNTrainer.CONV_STRIDES_W, 1),
        ...
        def max_pool3d(
                x, ksize,
                strides=(1, CNNTrainer.POOL_STRIDES_D, CNNTrainer.POOL_STRIDES_H, CNNTrainer.POOL_STRIDES_W, 1),
                padding='SAME', name=None,
        ):
        ...
        with tf.name_scope('model') as _:
            ...
            with tf.name_scope('InputLayer') as _:
                in0 = x
                x_image = tf.reshape(in0, [-1, initial_depth, initial_height, initial_width, initial_channels],
                                     name='x_image', )
                out0 = x_image
            ...
            with tf.name_scope('S2') as _:
                ...
                h_pool = max_pool3d(_in, CNNTrainer.POOL_SHAPE, name='h_pool', )
            ...
            with tf.name_scope('F6') as _:
                ...
                W_fc = weight_variable([_depth * _height * _width * _in_channels, _out_width], name='W_fc', )
                b_fc = bias_variable([_out_width], name='b_fc', )
                h_pool_flat = tf.reshape(_in, [-1, _depth * _height * _width * _in_channels], name='h_pool_flat', )
    ...
    def add_image2summary(x, image_name_prefix):
        ...
        for depth_no in range(depth):
            for channel_no in range(channels):
                # FIXME 20171011 may cover old images in a same layer??
                image = x[:, depth_no:depth_no + 1, :, :, channel_no:channel_no + 1]
                image = tf.reshape(image, [-1] + map(lambda _: _.value, image.get_shape()[2:]))
                image_name = '%s-%d-%d' % (image_name_prefix, depth_no, channel_no,)
